...
`wait_for_job`, `fetch_artifacts`, `cancel_job`, `lab_fairshare`. Together they run
...
4. **Produce.** Scale up only what the sanity gate validated. Prefer one job that
   sweeps internally (SweepBox pattern) over many jobs. Long jobs: submit, tell
   the user the job id and ETA, and poll `job_status` rather than blocking.
...
Max 4 concurrent jobs, ≤16 CPUs, ≤64 GB, ≤48 h, partitions shared/test/serial_requeue.
